...
| [`orx/evaluator-visible-release-candidate`](https://github.com/MachineLearning-Nerd/icml26-repro-okzQ1x71pS-generalizing-stochastic-smoothing-for-differentiation-and-gradient-estimatio/tree/orx/evaluator-visible-release-candidate) | Pages, raw data, report, notebook, release checks | `uv sync --frozen --no-dev && .venv/bin/python -m repro.run` | Pending release regression | HF `cpu-upgrade`, pending |
