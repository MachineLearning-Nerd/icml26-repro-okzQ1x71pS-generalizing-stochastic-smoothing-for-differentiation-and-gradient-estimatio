...
This was over-strict because equality of binary encodings is not equality of shortest-path optima when ties or serialization uncertainty exist. The current verifier in `repro/check_warcraft.py` instead requires valid paths and objective-cost compatibility under the published half-precision weights, while reporting exact encoding separately.
